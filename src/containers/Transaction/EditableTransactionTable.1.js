...
import { EditableContext, getTransactionsEndpoint, deleteTransactionsEndpoint, addTransactionEndpoint, updateTransactionEndpoint, EditableFormRow, mapStateToProps, EditableCell } from './EditableTransactionTable';


// eslint-disable-next-line import/prefer-default-export
export class EditableTransactionTable extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      pagination: {},
          ...
          return (<div>
            {editable ? (<span>
              <EditableContext.Consumer>
                {form => (<a href="javascript:;" onClick={() => this.save(form, record._id)} style={{ marginRight: 8 }}>
                  Save
                        </a>)}
              </EditableContext.Consumer>
            ...
            </span>) : (<a onClick={() => this.edit(record._id)}><Icon type="edit" /></a>)}
          </div>);
        },
      },
    ];
      ...
      if (this.state.addFlag) {
        ...
        if (index > -1) {
          ...
          this.props.transactions.splice(index, 1, {
            _id: item._id,
            transactionDate: moment(row.transactionDate).format('MMM DD, YYYY'),
  ...
  render() {
    ...
    return (<div>
      ...
      <Button disabled={(this.props.count < 1 && this.state.selectedRowKeys.length < 1) ||
        (this.props.count > 0 && this.state.selectedRowKeys.length < 1)} onClick={this.onDeleteRecord} type="primary" style={{ marginBottom: 16, marginLeft: 20, marginRight: 30 }}>
        Delete

        </Button>

      <Table rowSelection={rowSelection} components={components} bordered dataSource={this.props.transactions} columns={columns} onRow={this.onRow} onChange={this.onhandleChangePage} pagination={{
        pageSize: 10,
